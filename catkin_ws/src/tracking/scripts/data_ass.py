...
class ourFilter():
	...
	def reciever_of_data(self, msg):
		...
		targetFrame = "odom"
		try:
			trans = self.tfBuffer.lookup_transform(targetFrame, msg.header.frame_id, time_now, rospy.Duration(1))
			...
			trans2 = self.tfBuffer.lookup_transform(targetFrame, "flat_body", time_now, rospy.Duration(1))
			next_tuple = (time_now.to_sec(), newPoint.point.x, newPoint.point.y, newPoint.point.z, trans2.transform.translation.x, trans2.transform.translation.y, trans2.transform.translation.z)
		...
		while not rospy.is_shutdown():
			if (time.time() - self.last_time) > 5:
				...
				for x in range(len(self.data)):
					self.data[x] = (self.data[x][0] - first_time, self.data[x][1], self.data[x][2], self.data[x][3], self.data[x][4], self.data[x][5], self.data[x][6]) 
