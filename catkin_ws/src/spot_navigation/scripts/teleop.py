...
def genTests():
	...
	testList.append(genRampTest(30, 0, 1.6, 10))
...
def cb(data):
	...
	# vel_limit buttons
	if data.buttons[1]:
		rospy.set_param('controller', 'pid')
		rospy.loginfo('Set PID control')
		pass # to disable start of testing !!!!!!!########!!!!!!!!!!
		# global not_ready_for_test
		# if not_ready_for_test:
		# 	not_ready_for_test = False
	if data.axes[7] > 0.9 and not_ready_for_test:
	...
	if data.buttons[8] and not_ready_for_test:
		if rospy.Time.now().to_sec() - last_vel_change_time > change_vel_limit_delay:
			rospy.set_param('controller', 'none')
			rospy.loginfo('Set manual control')
			last_vel_change_time = rospy.Time.now().to_sec()
			vel_limit = 1.5
			vel_limit_pub.publish(Float32(vel_limit))
...
def listener():
	...
	while not rospy.is_shutdown():
		...
		for test in testList:
			while not_ready_for_test and not rospy.is_shutdown():
				rospy.sleep(1)
			print('starting test!')
